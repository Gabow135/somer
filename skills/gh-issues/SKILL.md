...
user-invocable: true
metadata:
  { "somer": { "requires": { "bins": ["curl", "git", "gh"] }, "primaryEnv": "GH_TOKEN" } }
---
...
IMPORTANT — No `gh` CLI dependency. This skill uses curl + the GitHub REST API exclusively. The GH_TOKEN env var is already injected by SOMER. Pass it as a Bearer token in all API calls:
...
```
cat ~/.somer/somer.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```

If still empty, check `/data/.clawdbot/somer.json`:

```
cat /data/.clawdbot/somer.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the SOMER dashboard or in ~/.somer/somer.json under skills.entries.gh-issues."
...
- **Sequential cursor tracking:** Use a cursor file to track which issue to process next:
  ...
  # SOURCE_REPO_SLUG = owner-repo with slashes replaced by hyphens (e.g., somer-somer)
  ```
...
GH_TOKEN=$(cat ~/.somer/somer.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.clawdbot/somer.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.clawdbot/somer.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")

```
If that fails, also try:
```

export GH_TOKEN=$(cat ~/.somer/somer.json 2>/dev/null | node -e "const fs=require('fs');const d=JSON.parse(fs.readFileSync(0,'utf8'));console.log(d.skills?.entries?.['gh-issues']?.apiKey||'')")
...
```
DEVOPS — Issues procesados | owner/repo | 26/Mar/2026

RESULTADO
  Issues procesados: 3 | PRs abiertos: 3 | Reviews pendientes: 1

ITEMS
  [x] #45 Fix auth token refresh — PR #52 abierto | Gabriel
  [x] #46 Add rate limiting — PR #53 abierto | Gabriel
  [!] #47 Update dependencies — PR #54 en revisión | Gabriel

---
Fuente: GitHub | Repo: owner/repo
```
