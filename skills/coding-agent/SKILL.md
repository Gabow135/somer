...
metadata:
  {
    "somer": { "emoji": "🧩", "requires": { "anyBins": ["claude", "codex", "opencode", "pi"] } },
  }
---
...
**⚠️ CRITICAL: Never review PRs in SOMER's own project folder!**
...
8. **NEVER start Codex in ~/.somer/** - it'll read your soul docs and get weird ideas about the org chart!
9. **NEVER checkout branches in ~/Projects/somer/** - that's the LIVE SOMER instance!
...
For long-running background tasks, append a wake trigger to your prompt so SOMER gets notified immediately when the agent finishes (instead of waiting for the next heartbeat):

```
... your task here.

When completely finished, run this command to notify me:
somer system event --text "Done: [brief summary of what was built]" --mode now
```
...
When completely finished, run: somer system event --text \"Done: Built todos REST API with CRUD endpoints\" --mode now'"
```
...
```
ACCIÓN — Coding Agent | Tarea delegada | 26/Mar/2026

RESULTADO
  Estado:     Completado
  Detalle:    Tarea delegada a Claude Code en background
...
---
Ejecutado por: SOMER Coding Agent
```
