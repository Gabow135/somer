---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for SOMER deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, SOMER cron scheduling for periodic checks, or version status checks on a machine running SOMER (laptop, workstation, Pi, VPS).
---

# SOMER Host Hardening

## Overview

Assess and harden the host running SOMER, then align it to a user-defined risk tolerance without breaking access. Use SOMER security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim SOMER changes the host firewall, SSH, or OS updates; it does not.
...
5. SOMER gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if SOMER can access sensitive data.
...
### 2) Run SOMER security audits (read-only)

As part of the default read-only checks, run `somer security audit --deep`. Only offer alternatives if the user requests them:

1. `somer security audit` (faster, non-probing)
2. `somer security audit --json` (structured output)

Offer to apply SOMER safe defaults (numbered):

1. `somer security audit --fix`

Be explicit that `--fix` only tightens SOMER defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check SOMER version/update status (read-only)

As part of the default read-only checks, run `somer update status`.
...
- Credential hygiene notes (location of SOMER creds, prefer disk encryption)
...
After SOMER install or first hardening pass, run at least one baseline audit and version check:

- `somer security audit`
- `somer security audit --deep`
- `somer update status`

Ongoing monitoring is recommended. Use the SOMER cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `somer cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `somer update status`
...
Before creating, `somer cron list` and match on exact `name`. If found, `somer cron edit <id> ...`.
If not found, `somer cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `somer update status` (preferred for source checkouts and channels)
2. `npm view somer version` (published npm version)

## SOMER command accuracy

Use only supported commands and flags:

- `somer security audit [--deep] [--fix] [--json]`
- `somer status` / `somer status --deep`
- `somer health --json`
- `somer update status`
- `somer cron add|list|runs|run`

Do not invent CLI flags or imply SOMER enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by SOMER compaction:
...
```
ESTADO — SOMER Healthcheck | 26/Mar/2026

ESTADO GENERAL: OK

CHECKS
  [OK] Host hardening: Firewall activo, SSH key-only
  [OK] Dependencias: Todas actualizadas
  [!]  Permisos: ~/.somer/credentials/ tiene permisos 755 (debería ser 700)
  [OK] Config: Validación OK, sin secretos expuestos

MÉTRICAS
  Risk score:  2/10 (bajo)
  Checks:      3/4 passed

ALERTAS
  [!] Corregir permisos: chmod 700 ~/.somer/credentials/

---
Verificado por: SOMER | Servicio: Healthcheck
```
