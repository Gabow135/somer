---
name: things-mac
description: Manage Things 3 via the `things` CLI on macOS (add/update projects+todos via URL scheme; read/search/list from the local Things database). Use when a user asks SOMER to add a task to Things, list inbox/today/upcoming, search tasks, or inspect projects/areas/tags.
...
- If DB reads fail: grant **Full Disk Access** to the calling app (Terminal for manual runs; `SOMER.app` for gateway runs).
...
```
RECORDATORIOS — Things 3 | 26/Mar/2026

RESUMEN: 12 | 6 pendientes | 2 vencidos

VENCIDOS
  [!!] Enviar propuesta a Acme — vencido 24/Mar/2026 | Trabajo
  [!!] Actualizar CV — vencido 25/Mar/2026 | Personal

HOY
  [ ] Revisar PR de autenticación — Proyecto SOMER
  [ ] Preparar slides para demo — Proyecto Acme

PRÓXIMOS
  [ ] Deploy producción v2.1 — 28/Mar/2026 | Proyecto SOMER
  [ ] Reunión trimestral — 01/Abr/2026 | Trabajo

---
Fuente: Things 3 | Actualizado: 26/Mar/2026 09:15
```
