...
1. Download the runtime for your OS (extracts into `~/.somer/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `~/.somer/tools/sherpa-onnx-tts/models`)

Update `~/.somer/somer.json`:
...
```json5
{
  skills: {
    entries: {
      "sherpa-onnx-tts": {
        env: {
          SHERPA_ONNX_RUNTIME_DIR: "~/.somer/tools/sherpa-onnx-tts/runtime",
          SHERPA_ONNX_MODEL_DIR: "~/.somer/tools/sherpa-onnx-tts/models/vits-piper-en_US-lessac-high",
        },
      },
    },
  },
...
```
MEDIA — Audio generado (TTS) | 26/Mar/2026

RESULTADO
  Tipo:       Audio (TTS)
  Archivo:    tts_output.wav
  Formato:    WAV
  Duración:   00:15

DETALLES
  Modelo:     sherpa-onnx (local)
  Voz:        es_MX-claude
  Texto:      "Buenos días, tu briefing del día está listo"

---
Procesado por: SOMER Media | Herramienta: sherpa-onnx
```
